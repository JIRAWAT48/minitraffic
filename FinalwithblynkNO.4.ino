...
#define BLYNK_TEMPLATE_ID "TMPL6qxzkUhHh"
#define BLYNK_TEMPLATE_NAME "mini smart traffic"
#define BLYNK_AUTH_TOKEN "WHn376-A9riSCRVlR0jmuJ4qOxzHK8Lc"
...
void setup() {
    ...
    // รอจนกว่า WiFi เชื่อมต่อ
    while (WiFi.status() != WL_CONNECTED) {
        delay(500);
        Serial.print(".");
    }
    Serial.println();
    Serial.println("เชื่อม WiFi ได้แล้ว");
...
void loop() {
    ...
    if (digitalRead(IR_SENSOR1) == LOW || digitalRead(IR_SENSOR2) == LOW) {
        activatePedestrianCrossing();
    }

    // ส่งข้อมูลไป Blynk
    Blynk.virtualWrite(V0, totalCars);
    Blynk.virtualWrite(V1, redLightViolations);
    Blynk.virtualWrite(V2, zebraCrossViolations);

    float redLightRatio = 0.0;
    if (totalCars > 0) {
        redLightRatio = ((float)redLightViolations / totalCars) * 100.0;
    }
    Blynk.virtualWrite(V3, redLightRatio);  // ส่ง % รถฝ่าไฟแดง
